...
def recall(pos_index, pos_len):
    r"""Recall_ (also known as sensitivity) is the ratio of the relevant items found in the recommended list
    to the total number of relevant items.

    .. _Recall: https://en.wikipedia.org/wiki/Precision_and_recall

    .. math::
        \mathrm {Recall@K} =\frac{|Rel \cap Rec@K|}{|Rel|}

    :math:`Rel` is the set of all relevant items.
    :math:`Rec@K` is the set of K recommended items.
    :math:`|Rel \cap Rec@K|` is the number of relevant items in the recommended list.
    """

    hits = np.cumsum(pos_index, axis=1)  # 累计命中数

    recall_result = hits / np.maximum(pos_len[:, np.newaxis], 1)
