...
def load_data(data_dir, dataset, device, batch_size):
    ...
    # bsg_only_types = statistics.get('bsg_only_types', [])  # <<< 改动位置 1
    ...
    for behavior_type in ['ubg'] + bsg_types + trbg_types:
        ...
        if edge_list.size == 0:
            print(f"[WARNING] Skipping empty file: {behavior_type}.txt")
            continue  # 跳过本次循环，处理下一个行为
        edge_list = torch.from_numpy(edge_list).to(device).T
        edge_dict[behavior_type] = convert_edge(edge_list, n_users)

    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # 单独加载新增的图（view_and_buy、cart_and_buy、collect_and_buy）
    for behavior_type in ['view_and_buy', 'cart_and_buy']:
        if os.path.exists(f'{data_dir}/{behavior_type}.txt'):
            edge_list = np.loadtxt(f'{data_dir}/{behavior_type}.txt', dtype=int)
            edge_list = torch.from_numpy(edge_list).to(device).T
            edge_dict[behavior_type] = convert_edge(edge_list, n_users)

    if 'collect' in bsg_types:
        behavior_type = 'collect_and_buy'
        if os.path.exists(f'{data_dir}/{behavior_type}.txt'):
            edge_list = np.loadtxt(f'{data_dir}/{behavior_type}.txt', dtype=int)
            edge_list = torch.from_numpy(edge_list).to(device).T
            edge_dict[behavior_type] = convert_edge(edge_list, n_users)
